...
export class CommunityService {
	...
	async findOneBySlug(slug: string): Promise<Community> {
		return this.communityRepository.findOne({ where: { slug }, relations: ['discordServer'], });
	}

	async findOneByName(name: string): Promise<Community> {
		return this.communityRepository.findOne({ where: { name }, relations: ['discordServer'], });
	}

	async findOneById(id: number): Promise<Community> {
		return this.communityRepository.findOne({ where: { id }, relations: ['discordServer'], });
	}
	...
	async updateDiscordGuildId(id: number, discordServer: DiscordServer): Promise<Community> {
