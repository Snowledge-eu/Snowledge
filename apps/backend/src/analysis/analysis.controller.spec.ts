...
describe('AnalysisController', () => {
	...
	describe('POST /analysis/discord', () => {
		...
		it('should successfully analyze Discord messages', async () => {
			...
			const result = await controller.analyzeDiscord(mockDto, {
				set: jest.fn(),
				status: jest.fn(),
			} as any);

			expect(analysisProvider.analyzeDiscord).toHaveBeenCalledWith(
				mockDto,
			);
		...
		it('should rethrow other HttpExceptions', async () => {
			...
			await expect(
				controller.analyzeDiscord(mockDto, {
					set: jest.fn(),
					status: jest.fn(),
				} as any),
		...
		it('should handle custom period with startDate and endDate', async () => {
			const customDto: DiscordAnalyzeDto = {
				creator_id: 1,
				serverId: 123456789,
				channelId: '987654321',
				prompt_key: 'test-prompt',
				period: AnalyzePeriod.CUSTOM,
			...
			const result = await controller.analyzeDiscord(customDto, {
				set: jest.fn(),
				status: jest.fn(),
			} as any);
	...
	describe('POST /analysis', () => {
		...
		it('should find analysis by scope', async () => {
			...
			expect(analysisProvider.findByScope).toHaveBeenCalledWith(
				mockFindDto,
			);
			expect(result).toEqual(mockResult);
		});
	...
	describe('PATCH /analysis/:id', () => {
		...
		it('should throw NotFoundException when analysis not found', async () => {
			...
			await expect(controller.update('1', mockUpdateDto)).rejects.toThrow(
				NotFoundException,
			);
		});
	});
