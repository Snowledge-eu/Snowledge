          ...
          result?.trends.map((trend: any, index: number) => (
            <AccordionItem value={`trend-${index}`} key={index}>
              ...
              <AccordionContent className="space-y-3">
                ...
                <Link
                  href={`/${slug}/summary?analysisId=${result._id?.toString()}&trendId=${index}`}
                >
                  <Button
  ...
  trends: [
    {
      title: "Bot Downtime",
      summary:
        "Users discussed the recent downtime of the moderation bot and its impact on server moderation. Suggestions for alternative tools were shared.",
      activity_level: "High",
    ...
    },
    {
      ...
      summary:
        "Excitement and questions about the new voting system. Some users asked for a tutorial, others debated its fairness.",
    ...
    },
    {
      ...
      summary:
        "Community members shared questions and excitement about the upcoming AMA session with the project founders.",
      activity_level: "Low",
      representative_messages: [
